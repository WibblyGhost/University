...
# ToDo: 4.0.* Find the running time of these algorithms
def main():
    open('results.csv', 'w').close()
    start_time = time.time()
    test_files_a = sorted(glob.glob('A_*.dat'), key=lambda x: int(x[2:-4]))
    test_files_b = sorted(glob.glob('B_*.dat'), key=lambda x: int(x[2:-4]))

    # Testing Gift Wrapping
    test_giftwrap(test_files_a)
    test_giftwrap(test_files_b)

    # Testing Graham Scan
    test_grahamscan(test_files_a)
    test_grahamscan(test_files_b)

    # Testing Monotone Chain
    test_chain(test_files_a)
    test_chain(test_files_b)
    print(time.time() - start_time)


def test_giftwrap(test_files):
    with open('results.csv', 'a') as csvfile:
        csvfile.write('giftwrap, Output, Time\n')
        for inp in test_files:
            start_time = time.time()
            data = giftwrap(readDataPts(inp, 100000000))
            data = str(data).replace(',', ';')
            csvfile.write('{}, {}, {}\n'.format(inp, data, time.time() - start_time))


def test_grahamscan(test_files):
    with open('results.csv', 'a') as csvfile:
        csvfile.write('grahamscan, Output, Time\n')
        for inp in test_files:
            start_time = time.time()
            data = grahamscan(readDataPts(inp, 100000000))
            data = str(data).replace(',', ';')
            csvfile.write('{}, {}, {}\n'.format(inp, data, time.time() - start_time))


def test_chain(test_files):
    with open('results.csv', 'a') as csvfile:
        csvfile.write('monotonechain, Output, Time\n')
        for inp in test_files:
            start_time = time.time()
            data = chain(readDataPts(inp, 100000000))
            data = str(data).replace(',', ';')
            csvfile.write('{}, {}, {}\n'.format(inp, data, time.time() - start_time))
